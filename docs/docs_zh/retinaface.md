...
模型下载方式：
```bash
python3 -m dfss --url=open@sophgo.com:sophon-pipeline/models/retinaface.tar.gz 
```
参数说明
...
# ./x86/retinaface_demo --help 查看命令行帮助信息
...
# ./soc/retinaface_demo --help 查看命令行帮助信息
# 以arm SoC 1684x为例
...
```bash
cd ${SOPHON_PIPELINE}/release/retinaface_demo
# ./arm64/retinaface_demo --help 查看命令行帮助信息
# 以arm pcie 1684x为例,将下载好的retinaface模型拷贝到${SOPHON_PIPELINE}/release/retinaface_demo目录下运行
