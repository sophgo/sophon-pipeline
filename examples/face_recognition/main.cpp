...
int main(int argc, char *argv[])
{
    const char *keys="{help | 0 | Print help info}"
                     "{config | ./cameras_face_recognition.json | path to cameras_face_recognition.json}";
    ...
    for(int card_idx = 0; card_idx < card_num; ++card_idx) {
        ...
        std::shared_ptr<FaceDetector> det1 = std::make_shared<FaceDetector>(contextPtr);
        // model thresholds
        det1->set_nms(model_cfg.nms_threshold);
        det1->set_obj(model_cfg.obj_threshold);
        std::shared_ptr<FaceLandmark> det2 = std::make_shared<FaceLandmark>(contextPtr);
        std::shared_ptr<FaceExtract> det3 = std::make_shared<FaceExtract>(contextPtr);
        OneCardInferAppPtr appPtr = std::make_shared<OneCardInferApp>(appStatis, gui,
                                                                      tqp, contextPtr, model_cfg.output_path,
                                                                      start_chan_index, channel_num, 
                                                                      model_cfg.skip_frame, 3);
