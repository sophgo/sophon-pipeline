...
FaceLandmark::FaceLandmark(bm::BMNNContextPtr bmctx):m_bmctx(bmctx)
{
    auto net_name = bmctx->network_name(1);
    m_bmnet = bm::BMNNNetwork::create(bmctx->bmrt(), net_name);//"LNet_v1"
    m_inputSize.width = 48;
    m_inputSize.height = 48;
    MAX_BATCH = m_bmnet->inputTensor(0)->get_shape()->dims[0];
}
    ...
    for(int frameInfoIdx = 0; frameInfoIdx < frame_infos.size(); ++frameInfoIdx) {
        ...
        for (int batch_idx = 0; batch_idx < batch_num; ++batch_idx) {
            ...
            if (inputTensorPtr->get_dtype() == BM_INT8) {
                alpha = m_alpha_int8 * inputTensorPtr->get_scale();
                beta = m_beta_int8 * inputTensorPtr->get_scale();
...
{
    float conv_scale_to_float = m_bmnet->get_output_scale(0);
    float prob_scale_to_float = m_bmnet->get_output_scale(1);
    ...
    for(int frameInfoIdx = 0; frameInfoIdx < frame_infos.size(); ++frameInfoIdx) {
        ...
        for(int fwd_idx = 0; fwd_idx < frame_info.forwards.size(); ++fwd_idx) {
            ...
            bm::BMNNTensorPtr points_tensor = get_output_tensor("conv6-2", &frame_info.forwards[fwd_idx], conv_scale_to_float);
            bm::BMNNTensorPtr prob_tensor = get_output_tensor("prob1", &frame_info.forwards[fwd_idx], prob_scale_to_float);
