...
int main(int argc, char *argv[])
{
    const char *base_keys="{help | 0 | Print help information.}"
                          ...
                          "{decode_yuv420p | false | decode yuv420p}"
    ...
    bool decode_yuv420p = parser.get<bool>("decode_yuv420p");
    ...
    for(int card_idx = 0; card_idx < card_num; ++card_idx) {
        ...
        appPtr->set_decode_yuv420p(decode_yuv420p);
        ...
        profile_thread[card_idx] = std::thread([&](){
            ...
            while(true){
                ...
                int vpu_usage[6], jpu_usage[5], vpp_usage[5];
                memset(vpu_usage, 0, sizeof(int)*6);
                memset(jpu_usage, 0, sizeof(int)*5);
                memset(vpp_usage, 0, sizeof(int)*5);
