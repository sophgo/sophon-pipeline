    ...
    bm::Watch *w) : w_(w)
{
    ...
    if (input_shape.num_dims != 4)
    {
        ...
        std::cout << "[ERROR] invalid input dims: " << input_shape.num_dims <<std::endl;
...
/*
bm_image Retinaface::read_image(bm::cvs11FrameBaseInfo &frame)
{
    bm_handle_t handle = impl_->ctx->handle();
    bm_image image;
    ...
    if (w > vpp_limit || h > vpp_limit)
    {
        ...
        int strides[3];
        const bm_image_format_ext format = FORMAT_BGR_PACKED;
        strides[0] = align_with(w*3, 64);
...
void Retinaface::decode_process(bm::cvs11FrameBaseInfo &frame)
{
    bm_handle_t handle = impl_->ctx->handle();
...
int Retinaface::preprocess(
    ...
    std::vector<bm::cvs11FrameInfo> &frame_infos)
{
    ...
    for (int i = 0; i < frames.size(); i += impl_->batch_size)
    {
        ...
        for (int j = 0; j < num; ++j)
        {
            ...
            //} 
                ...
                // LOG(FATAL) << "No input";
                std::cout << "[FATAL] No input" << std::endl;
            }
            ...
            if (w_) w_->mark("vpp");
            //bm_image stride_image;
            //bm::BMImage::bm_images_clone(handle,
            //                             &image,
            //                             1,
            //                             &stride_image,
            //                             64,
            ...
            //bm_image_destroy_batch(&stride_image, 1);
            if (w_) w_->mark("vpp");
...
void saveTensor1(bm_handle_t& handle,const std::string& file_path,bm_tensor_t& tensor){
    int rank = 1;
    for(int i=0;i<tensor.shape.num_dims;i++){
        rank*=tensor.shape.dims[i];
    }
    std::vector<uint8_t> tensor_bin(rank*sizeof(float));
    call(
    bm_memcpy_d2s_partial_offset,
    handle, tensor_bin.data(), tensor.device_mem,
    rank*sizeof(float), 0);
    
    FILE * fp = fopen(file_path.c_str(),"wb");
    fwrite(tensor_bin.data(),1,rank*sizeof(float),fp);
    fclose(fp);
}

void writeTensor(bm_handle_t& handle,const std::string& file_path,bm_tensor_t& tensor){
    int rank = 1;
    for(int i=0;i<tensor.shape.num_dims;i++){
        rank*=tensor.shape.dims[i];
    }
    ...
    fread(tensor_bin.data(),1,rank*sizeof(float),fp);
    fclose(fp);
            call(
            bm_memcpy_s2d_partial_offset,
            handle, tensor.device_mem, tensor_bin.data(),
            rank*sizeof(float), 0);
}

int Retinaface::forward(std::vector<bm::cvs11FrameInfo> &frame_infos)
{
    ...
    for (int b = 0; b < frame_infos.size(); ++b)
    {
        ...
        //saveTensor1(handle,"./i.dat",input_tensor);
        //writeTensor(handle,"./pre-realai.dat",input_tensor);
#if 0
        bm_handle_t handle = impl_->ctx->handle();
        auto &input_tensor = frame_info.input_tensors[0];
        ...
        if (bmrt_tensor_bytesize(&input_tensor) % bin->size())
        {
            ...
            std::cout << "[ERROR] invalid input.bin" << std::endl;
        }
...
int Retinaface::postprocess(std::vector<bm::cvs11FrameInfo> &frame_infos)
{
    ...
    bm::cvs11FrameInfo &frame_info)
{
    ...
    if (idx < 0 && idx + 1> output_tensor_num) {
        ...
        std::cout << "[ERROR] invalid index " << idx << std::endl;
    ...
    if (tensor.dtype != BM_FLOAT32)
    {
        ...
        std::cout << "[ERROR] " << "Output tensor not of FLOAT32\n"
            << "Please compile .bmodel with -output_as_fp32 option." << std::endl;
        throw std::runtime_error("invalid model");
    }
...
cond:
    ...
    for (auto &obj : objs)
    {
        // obj.x1 = round((obj.x1 - frame.x_offset) / frame.x_scale * input_width);
        // obj.x2 = round((obj.x2 - frame.x_offset) / frame.x_scale * input_width);
        // obj.y1 = round((obj.y1 - frame.y_offset) / frame.y_scale * input_height);
        // obj.y2 = round((obj.y2 - frame.y_offset) / frame.y_scale * input_height);
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            // x = (x - frame.x_offset) / frame.x_scale * input_width;
            // y = (y - frame.y_offset) / frame.y_scale * input_height;
        }
    }
    // sanity check
...
void saveOutput(const std::string& file_path,const float * data, const int length){

    FILE * fp = fopen(file_path.c_str(),"wb");
    fwrite(data,1,length*sizeof(float),fp);
    fclose(fp);
}
...
void Retinaface::extract_facebox_cpu(
    bm::cvs11FrameInfo &frame_info)
{
    bm::BMNNTensorPtr land_tensor, cls_tensor, loc_tensor;
    int output_tensor_num = frame_info.output_tensors.size();
    for (int i = 0; i < output_tensor_num; ++i)
    {
        auto &tensor = frame_info.output_tensors[i];
        if (tensor.dtype != BM_FLOAT32)
        {
            // LOG(ERROR) << "Output tensor not of FLOAT32\n"
            //            << "Please compile .bmodel with -output_as_fp32 option.";
        ...
        bm::BMNNTensorPtr *t;
        std::string name;
        if (tensor.shape.dims[2] == 4)
        {
            name = "loc";
            t = &loc_tensor;
        } else if (tensor.shape.dims[2] == 10)
        {
            name = "land";
            t = &land_tensor;
        } else {
        ...
        *t = std::make_shared<bm::BMNNTensor>(
            impl_->ctx->handle(), name, scale, &tensor);
    }
    ...
    // saveOutput("./land.dat",land_data,land_tensor->total());
    // saveOutput("./cls.dat",cls_data,cls_tensor->total());
    // saveOutput("./loc.dat",loc_data,loc_tensor->total());
