...
int FaceExtract::preprocess(std::vector<bm::FeatureFrame> &frames, std::vector<bm::FeatureFrameInfo> &of)
{
    ...
    for(int batch_idx = 0; batch_idx < batch_num; ++ batch_idx) {
        ...
        convert_to_attr.alpha_0 = alpha * input_scale;
        convert_to_attr.alpha_1 = alpha * input_scale;
        convert_to_attr.alpha_2 = alpha * input_scale;
        convert_to_attr.beta_0  = beta * input_scale;
        convert_to_attr.beta_1  = beta * input_scale;
        convert_to_attr.beta_2  = beta * input_scale;
...
void FaceExtract::extract_facefeature_cpu(bm::FeatureFrameInfo &frame_info) {
    ...
    for(int frameIdx = 0; frameIdx < frameNum;++frameIdx) {
        bm::BMNNTensorPtr output_tensor = get_output_tensor("52", frame_info, scale);
